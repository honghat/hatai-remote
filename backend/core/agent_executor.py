...
MAX_ITERATIONS = 2000
PLAN_MAX_TOKENS = 400
PLAN_BYPASS_LENGTH = 45  # Skip planning for very short prompts
...
def _parse_tool_calls(content: str) -> List[tuple]:
    ...
    # Strategy 2: If no code blocks, try raw JSON objects anywhere in content
    if not code_blocks:
    ...
    # Strategy 3: If still nothing, try to find "tool": "xxx" pattern and extract surrounding JSON
    if not code_blocks:
...
def _generate_plan_stream(engine, user_message, 
    ...
    """
    plan_prompt = f"""Analyze this task and create an execution plan.
Task: {user_message}
Rules:
- If simple (1 tool), reply: SIMPLE
- Otherwise, break into numbered steps. Each step = 1 tool call.
- Format: 1. [tool_name] description
1. [tool_name] ..."""

    plan_messages = [
        {"role": "system", "content": "You are a precise task planner. Output ONLY numbered steps or SIMPLE."},
...
def run_agent(user_message, history=None, max_tokens=4096, temperature=0.5, cancel_event=None, session_id=None):
    ...
    try:
        ...
        plan_steps = []
        is_complex = len(user_message) > PLAN_BYPASS_LENGTH or any(word in user_message.lower() for word in ["tìm", "search", "quét", "lọc", "code", "viết", "tạo", "sửa"])
        
        if is_complex:
            yield {"type": "thinking", "content": "Đang phân tích yêu cầu..."}
            plan_text = ""
            for chunk, final in _generate_plan_stream(engine, user_message):
...
YOUR PLAN (follow step by step, do NOT skip steps):
{chr(10).join(plan_steps)}

Execute step 1 NOW. Call the tool."""
        else:
        ...
        for i in range(MAX_ITERATIONS):
            ...
            if engine._context_overflow or resp.startswith("[CONTEXT_OVERFLOW]"):
                ...
                if overflow_count > 3:
                    ...
                    return
                yield {"type": "text", "content": f"⚠️ Context tràn (lần {overflow_count}), đang thu nhỏ và thử lại..."}
            ...
            if t_calls:
                ...
                if looks_like_tool:
                    ...
                    yield {"type": "error", "content": f"⚠️ Lệnh bị lỗi (lần {malformed_count}/2), đang thử lại..."}
                    continue
